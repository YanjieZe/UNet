...
import visdom
from sunrgbd_detection_dataset import bbox2points, apply_trans
import pickle
...
class sunrgbd_object(object):
    ...
    def __init__(self, root_dir, split='training', use_v1=False):
        ...
        self.seg_dir = os.path.join(self.split_dir, 'seg_points_label')
        if use_v1:
    ...
    def get_seg(self, idx):
        seg_filename = os.path.join(self.seg_dir, '%06d.mat'%(idx))
        return sio.loadmat(seg_filename)['instance']
...
def extract_sunrgbd_data(idx_filename, split, output_folder, num_point=20000,
    ...
    # vis = visdom.Visdom(port=20999)
    
    seg37list = sio.loadmat('OFFICIAL_SUNRGBD/SUNRGBDtoolbox/Metadata/seg37list.mat')
    
    seg37list = [a[0] for a in seg37list['seg37list'][0]]
    ...
    for data_idx in data_idx_list:
        ...
        segments = dataset.get_seg(data_idx)
        pc_upright_depth = dataset.get_depth(data_idx)
        pc_upright_depth_subsampled, choices = pc_util.random_sampling(pc_upright_depth, num_point, return_choices=True)
        segments_subsampled = segments[choices]
        
        object_list = []
        
        plots = pc_upright_depth[:, :3]
        ...
        for obj in objects:
            ...
            obj_idx = seg37list.index(obj.classname) + 1
            
            angle = obb[6]
            box3d_center = obb[0:3]
            box3d_size = obb[3:6]
            
            Mbbox = np.eye(4)
            
            # IMPORTANT: box3d is half of edge length
            Mbbox[:3, :3] = sunrgbd_utils.rotz(-angle) @ np.diag(box3d_size)
            Mbbox[:3, -1] = box3d_center
            
            bbox_corners = np.array([[-1., -1., -1.], [1., 1., 1.]])
            
            try:
                pc_inv = apply_trans(pc_upright_depth_subsampled[:, :3], np.linalg.inv(Mbbox))
                bbox_mask = (bbox_corners[0, 0] < pc_inv[:, 0]) & (pc_inv[:, 0] < bbox_corners[1, 0]) \
                            & (bbox_corners[0, 1] < pc_inv[:, 1]) & (pc_inv[:, 1] < bbox_corners[1, 1]) \
                            & (bbox_corners[0, 2] < pc_inv[:, 2]) & (pc_inv[:, 2] < bbox_corners[1, 2])
            except np.linalg.linalg.LinAlgError as err:
                bbox_mask = np.ones([pc_upright_depth_subsampled.shape[0],], dtype=np.bool)
            
            # import pdb; pdb.set_trace()
            label_mask = (segments_subsampled == obj_idx)[:, 0]
            ...
            bbox = apply_trans(np.stack([x_corners,y_corners,z_corners], 0).T, Mbbox)
            
            plots = np.concatenate([plots, bbox2points(bbox)])
            labels = np.concatenate([labels, np.ones((bbox2points(bbox).shape[0]), dtype=np.int) * 2])
            
            obj_idxs.append(np.where(label_mask & bbox_mask)[0])
            # obj_points = pc_upright_depth_subsampled[:, :3][label_mask & bbox_mask]
            # plots = np.concatenate([plots, obj_points])
            # labels = np.concatenate([labels, np.ones((obj_points.shape[0]), dtype=np.int) * 3])
        ...
        # vis.scatter(X=plots, Y=labels, win=1, opts=dict(markersize=3))
    
        # import pdb; pdb.set_trace()

        assert(len(obj_idxs) == len(obbs))
        ...
        pickle.dump(obj_idxs, open(os.path.join(output_folder, '%06d_segments.pkl' % (data_idx)), 'wb'))
